...
# TO-DO:
# 1. Format data from backend, its ugly
# 2. Add additional features. Due dates? Priority list?
# 3. Polish terminal interface -> Colors?
...
def view_tasks():
    ...
    if specificTask.lower() == 'n':
        ...
        view_tasks_response = requests.get(f"http://127.0.0.1:8000/tasks/{task_to_view}")
...
def remove_task():
    ...
    Selection: """)
        ...
        delete_task_response = requests.delete(f"http://127.0.0.1:8000/tasks/{task_to_delete}/")
        display_tasks(response=delete_task_response, display_multiple_tasks=True)
...
def update_task():
    ...
    if not task_to_update.isnumeric():
        ...
        update_task_response = requests.patch(f"http://127.0.0.1:8000/tasks/{task_to_update}")
...
def display_tasks(response: requests.Response, display_multiple_tasks: bool):
    ...
    if display_multiple_tasks == True:
        ...
        if response_dict.get("data") == True:
            status = "✔️"
            print(f"[{response_dict.get('task_id')}] {response_dict.get('task_name')} {status}")
...
def menuSelect():
    ...
    if menuResponse in menuOptions: menuOptions[menuResponse]()
        
while True: menuSelect()